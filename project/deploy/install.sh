...
#   1. Dynamically detects installation paths (NO hardcoded paths).
#   2. Verifies Python 3, venv, pip, and network discovery tools.
#   3. Enables and starts local MongoDB service if installed.
#   4. Initializes isolated Python virtual environment (venv) & dependencies.
#   5. Configures production environment (.env).
#   6. Installs and registers the systemd unit file (network-monitor.service).
#   7. Enables auto-start on server boot and auto-restart on crash (Restart=always).
#   8. Starts the service and verifies /system/health endpoint.
# ==============================================================================
...
# 2. Determine project and backend directories dynamically
